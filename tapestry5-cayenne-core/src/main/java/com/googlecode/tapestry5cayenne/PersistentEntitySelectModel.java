 ...
 * will be ordered according to the following rules: 1) If orderings are
 * explicitly provided via the varags constructor parameter, those orderings
 * will be used. 2) If the class type has a method annotated with the Label
 ...
 * not, the sort will occur in-memory. 3) If no label is found, but the class is
 * an instance of comparable, the objects will be sorted in-memory according to
 * their "comparable" ordering. 4) If no method can be found and the objects are
 * not comparable, no sorting will occur.
 * 
