 ...
 * Currently, "mandatory" (for both attributes and foreign keys) and maxlength
 * (for string-based properties: things that wind up as a CharSequence in the
 * ObjEntity) are supported.
 * 
 * @author robertz
 *
 */
public class CayenneConstraintGenerator implements ValidationConstraintGenerator {

  private final ObjectContextProvider provider;
  private Environment environment;

  public CayenneConstraintGenerator(final Environment environment, final ObjectContextProvider provider) {
  ...
  public List<String> buildConstraints(Class propertyType, AnnotationProvider provider) {
    ...
    // little tricky here. If it's a string, we add a maxlength validator,
  ...
  private boolean isPropRequired(DbAttribute dbatt, ObjRelationship rel) {
    ...
    // Otherwise, the child side of such a situation. Parent side: not
    // mandatory.
  ...
  private DbAttribute extractDbAttribute(String propId, ObjEntity oent, ObjRelationship rel) {
    if (rel == null) {
      if (oent.getAttribute(propId) == null) {
        return null;
      } else {
